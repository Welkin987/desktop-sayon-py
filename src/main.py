...
class DesktopSayon:
    def __init__(self):
        try:
            ...
            initial_transparency = float(self.config['AVATAR']['AVATAR_TRANSPARENCY'])
            self.root.attributes('-alpha', 1.0 - initial_transparency)
    ...
    def load_avatar(self):
        ...
        try:
            ...
            self.original_image = Image.open(avatar_path).convert("RGBA")
            ...
            # Resize image once
            self.resized_image = self.original_image.resize((avatar_width, avatar_height), Image.Resampling.LANCZOS)
            
            self.avatar_width = avatar_width
            self.avatar_height = avatar_height
            
            # Set the image without transparency modification
            self.avatar_image = ImageTk.PhotoImage(self.resized_image)
            
            print(f"Avatar processed: {avatar_width}x{avatar_height}")
    ...
    def on_mouse_drag(self, event):
        ...
        if current_time - self.drag_start_time > 0.3:
            ...
            # Update text window position if it exists
            if hasattr(self, 'text_window') and self.text_window and tk.Toplevel.winfo_exists(self.text_window):
                text_x = new_x
                text_y = new_y - 10  # 保持相对位置
                self.text_window.geometry(f"+{text_x}+{text_y}")
    ...
    def on_mouse_up(self, event):
        ...
        if self.dragging:
            ...
            # Update text window position if it exists
            if hasattr(self, 'text_window') and self.text_window and tk.Toplevel.winfo_exists(self.text_window):
                text_x = self.window_x
                text_y = self.window_y - 20  # 保持相对位置
                self.text_window.geometry(f"+{text_x}+{text_y}")
    ...
    def create_ui(self):
        ...
        # Initialize text window variable
    ...
    def show_text(self, text):
        """Display text in a semi-transparent pink dialog above avatar"""
        # Destroy existing text window if present
        ...
        self.text_window.attributes('-alpha', 0.8)
        self.text_window.configure(bg='#FFC0CB')
        # Create label inside
        label = tk.Label(
            self.text_window,
            text=text,
            bg='#FFC0CB',
            fg='#000000',
            font=('Microsoft YaHei', 10),  # 使用微软雅黑字体，更美观
            wraplength=300,
            justify='left',
            padx=12,
            pady=8
        ...
        # Position text window lower - closer to avatar
        x = self.window_x
        y = self.window_y - 10  # 进一步减少距离，让文字框更接近头像
    ...
    def hide_text(self):
        """Hide the semi-transparent dialog"""
        if self.text_window and tk.Toplevel.winfo_exists(self.text_window):
            self.text_window.destroy()
    ...
    def perform_dialog_action(self):
        ...
        def action():
            # Fade to transparent
            self.fade_avatar(0, lambda: self.continue_dialog_action())
            
        threading.Thread(target=action, daemon=True).start()
        
    def continue_dialog_action(self):
        """Continue dialog action after fade"""
        # Take screenshot
        self.take_screenshot()
        
        # Wait a moment for screenshot to be saved properly
        time.sleep(0.2)
        
        # Fade back to normal transparency
        original_transparency = float(self.config['AVATAR']['AVATAR_TRANSPARENCY'])
        target_alpha = 1 - original_transparency
        self.fade_avatar(target_alpha)
    ...
    def fade_avatar(self, target_alpha, callback=None):
        """Fade avatar to target alpha"""
        # Get current alpha from window attribute
        ...
        def fade_step(step):
            if step <= steps:
                ...
                self.root.after(25, lambda: fade_step(step + 1))
            elif callback:
                self.root.after(25, callback)  # Small delay before callback
